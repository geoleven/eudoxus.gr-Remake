<?php
	...
	if (isset($_SESSION["uem"])) {
		$stu = $dbu->executeQuery("SELECT sxoli FROM users WHERE email = \"".$_SESSION['uem']."\";");
		if ($stu->next()) {
			list($sxoli) = $stu->getCurrentValues();
			if ($sxoli == "") {
				$sxoli = "sxoli";
			}
			else {
			...
			$st = $db->executeQuery("SELECT * FROM books WHERE sxoli = ".$sxoli.";");
		}
	}
        ...
        <script type=text/javascript>
			function registerbook(myuid, mybookid) {
				...
				if (document.getElementById(mychkid).checked) {
					$.post ('./php/registerscript.php',{myuid: myuid, mybookid: mybookid});
				} else {
					$.post ('./php/unregisterscript.php',{myuid: myuid, mybookid: mybookid});
				}
			}
		</script>
        	...
        	<div id="mainsite">
            	...
            	<div id="boxes">                        
                 <div class="boxMain">
                         <div class="article">
                         	<?php
								if(!isset($_SESSION["uem"]) || $st->next() == 0) {
							?>
                            <br><br><br>Δεν μπορείτε να κάνετε δήλωση αν δεν συνδεθείτε ή δεν υπάρχουν βιβλία για την σχολή σας!<br><br><br>
                            <?php
								}
								else {
									list($idl, $namel, $ekdotisl, $isbnl, $writerl, $langl, $sxolil, $imagel) = $st->getCurrentValues();								
							?>
